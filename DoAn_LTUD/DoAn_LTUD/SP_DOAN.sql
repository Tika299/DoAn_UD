...
AS
BEGIN
    INSERT INTO Students (StudentID, FullName, Class, Gender, DateOfBirth)
...
AS
BEGIN
    SELECT StudentID, FullName, Class, Gender, DateOfBirth
    FROM Students;
END;
...
BEGIN
    SELECT 
        ...
        S.StudentID, 
        S.FullName, 
        S.Class, -- Lấy thông tin lớp từ bảng Student
        A.AwardName,
		A.AwardID,
        SA.AwardDate
    FROM StudentAwards SA
...
BEGIN
    ...
    WHERE 
        ...
        (@ClassName IS NULL OR S.Class LIKE '%' + @ClassName + '%') AND
...
BEGIN
    SELECT 
        ...
        s.Class,                     -- Mã lớp học (dùng thông tin từ bảng Students)
